/*!
 * @file DFRobot_LWLP_J.cpp
 * @brief Define the infrastructure of the DFRobot_LWLP_J class
 * @copyright   Copyright (c) 2010 DFRobot Co.Ltd (http://www.dfrobot.com)
 * @License     The MIT License (MIT)
 * @author [fengli](li.feng@dfrobot.com)
 * @version  V1.0
 * @date  2020-05-14
 * @url https://github.com/DFRobot/DFRobot_LWLP_J
 */

#include "DFRobot_LWLP_J.h"

DFRobot_LWLP_J::DFRobot_LWLP_J(TwoWire *pWire)
{
  _pWire = pWire;
  
}

int DFRobot_LWLP_J::begin(){
...
void DFRobot_LWLP_J::configChip(void){
  uint8_t config[3]={0xaa,0x00,0x80}; //Config list 
  write(config,3);//Transmit config data 
  delay(12);//delay 30 ms to wait the chip to collect and convert data 
}

DFRobot_LWLP_J::sLwlp_t DFRobot_LWLP_J::getData(void){//Get chip register data 
  uint8_t data[7];
  configChip();
...
DFRobot_LWLP_J::sLwlp_t DFRobot_LWLP_J::getfilterData(void){
  uint8_t readTime;
  uint8_t i,j,k;
...
void DFRobot_LWLP_J::write(const void* pBuf,size_t size)
{
  if (pBuf == NULL) {
...
uint8_t DFRobot_LWLP_J::readData(void *pBuf, size_t size) {
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
  uint8_t * _pBuf = (uint8_t *)pBuf;
